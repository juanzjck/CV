...
import {CarouselWrapp,Arrow} from './styles'
const  Skills = () => {
    ...
    const renderSkills = () => {
        let slides=[]
        skils.map((doc,Id)=>{
            slides.push(<Skill key={Id} category={doc.category} icon={doc.icon} title={doc.title} description={doc.description}/>) 
        })
        return slides
    }

    const onChange = (value) =>{
      if(value>0 && value<skils.length){
        setPosition(value)
      }
        ...
        return (  
            <div>
                ...
                <CarouselWrapp>
                    <Arrow  onClick={e=>onChange(position-1)}>
                        ...
                        <Carousel  
                            ...
                            'arrows',
                            'clickToChange'
                            ]} slides={renderSkills()}  className="carousel__container">
